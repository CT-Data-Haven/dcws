...
clean_cws_2020 <- function() {
  codes <- officer::read_docx("data-raw/misc_input/DataHaven0720_Prn2.docx") %>%
    ...
    dplyr::mutate(id = cumsum(style_name == "Alias"),
                  style_name = gsub("(?:^[\\w\\s]*?)(\\b\\w+\\b$)", "\\L\\1", style_name, perl = TRUE)) %>%
    dplyr::group_by(id, style_name) %>%
    dplyr::summarise(text = paste(text, collapse = " ")) %>%
    dplyr::ungroup() %>%
    tidyr::pivot_wider(id_cols = id, names_from = style_name, values_from = text) %>%
    dplyr::mutate(alias = stringr::str_remove(alias, "\\:\\s$"),
                  label = label %>%
  ...
  cws20 <- cwi::read_xtabs("data-raw/crosstabs/DataHaven2020 Connecticut Crosstabs.xlsx",
                           year = 2020, process = TRUE, code_pattern = "^$") %>%
    dplyr::distinct(q_number, question) %>%
    dplyr::mutate(label = question %>%
                    stringr::str_remove("\\[.+$") %>%
                    stringr::str_remove_all("\\(.+\\)") %>%
