...
test_that("clean_cws_lvls returns expected levels", {
    ...
    expect_false(any(
        c("Kids in home", "Children in HH") %in% levels(xt_grps$category)
    ))
    expect_false(any(grepl(
        "(,000|Less than \\$| or more|^Income|Towns| [Tt]otal|School|Under age|or older)",
        levels(xt_grps$group)
    )))
})
