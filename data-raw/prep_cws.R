...
paths <- list.files("data-raw/crosstabs", pattern = "\\.xlsx?", full.names = TRUE) |>
  ...
  dplyr::mutate(year = purrr::map_dbl(yrs, \(x) as.numeric(dplyr::last(x)))) |>
  dplyr::mutate(type = ifelse(lengths(yrs) > 1, "pooled", "single")) |>
...
cws_max_moe <- paths |>
  ...
  dplyr::select(-year) |>
  dplyr::mutate(year = purrr::map_chr(yrs, \(x) paste(x, collapse = "_"))) |>
